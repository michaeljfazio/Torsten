...
File: `crates/torsten-crypto/src/kes.rs`
...
Files: `crates/torsten-network/src/pipelined.rs`, `client.rs`, `miniprotocols/peersharing.rs`, `miniprotocols/txsubmission.rs`
...
File: `crates/torsten-serialization/src/multi_era.rs`
...
**Torsten's solution**: `PipelinedPeerClient` in `torsten-network/src/pipelined.rs` directly manipulates `ChannelBuffer` and `Plexer`/`RunningPlexer` internals. It manually encodes `chainsync::Message::RequestNext` using `ChannelBuffer::send_msg()` N times, then reads N `Message` responses.

**Impact**: 10-50x header throughput improvement. Default pipeline depth: 100 (configurable via `TORSTEN_PIPELINE_DEPTH`).
...
**Problem**: Pallas uses `Hash<28>` for pool IDs, DRep keys, required signers, pool voter keys. Torsten needs these as 32-byte hashes internally.
...
Torsten's Cargo.lock contains BOTH 0.33.0 and 1.0.0-alpha.5 versions of:
- pallas-addresses
- pallas-codec
- pallas-crypto
- pallas-primitives
...
Pallas provides `NodeServer` facade for N2C server functionality, but torsten implements its own complete N2C server. This is necessary because:
1. Torsten's N2C server implements custom LocalStateQuery with 38 query types
2. N2C V17-V22 with bit-15 version encoding extends beyond pallas's V16 support
3. Torsten needs direct control over the response encoding (integer CBOR keys, HFC wrapper logic)
