...
//! Run with:
//!   TORSTEN_INTEGRATION_SOCKET=./node.sock \
//!   TORSTEN_TEST_KEYS=keys/preview-test \
//!   cargo test -p torsten-integration-tests -- tier2

use std::fs;
use tempfile::TempDir;
use torsten_integration_tests::helpers::cli::{
    integration_socket, run_cli, run_cli_ok, test_keys_dir,
};
use torsten_integration_tests::helpers::keys::TempKeys;
use torsten_integration_tests::helpers::koios;
use torsten_integration_tests::helpers::wait::wait_for_confirmation;
use torsten_integration_tests::helpers::wallet::{get_utxos_cli, skip_if_underfunded};
...
fn require_env() -> (String, String, String, String) {
    ...
    let keys_dir = match test_keys_dir() {
        Some(d) => d,
        None => {
            eprintln!("SKIP: TORSTEN_TEST_KEYS not set");
            std::process::exit(0);
        }
    };
