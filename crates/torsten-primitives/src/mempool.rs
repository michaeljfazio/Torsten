...
//!
//! Defined in `torsten-primitives` so that both `torsten-mempool` (impl) and
//! `torsten-network` (consumer) can depend on it without creating a direct
...
///
/// This allows crates like `torsten-network` to interact with a mempool without
/// depending on the concrete `torsten-mempool` crate, keeping the coupling loose.
